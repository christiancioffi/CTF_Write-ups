...
Thus we can pickle any object (not only strings), encode it in base64 and set the "*order*" cookie accordingly. Once recieved by the server, the string will be decoded, the pickled object unpickled and, if a string, printed on the <code>/order</code> page.
